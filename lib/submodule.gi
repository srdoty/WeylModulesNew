...
function(W,vec)
 ...
 if vec = 0*Generator(W) then return SubWeylModule(W, []); fi;
...
function(W,vec)
 ...
 if vec = 0*Generator(V) then return(W); fi;     
...
#############################################################################
InstallMethod(SubWeylModule, "for a sub Weyl module and list of vectors", 
true, [IsSubWeylModule, IsList], 0, 
function(S,vecs)
 # Returns the sub Weyl module generated by <S> and the given vectors
 ...
 S:= SubWeylModule(S, vecs[1]); # to start
 for v in vecs{ [2..Length(vecs)] } do
   S:= SubWeylModule(S,v);
 od;
 return(S);
...
#############################################################################
InstallMethod(DecompositionNumbers, "for a sub Weyl module", true, 
[IsSubWeylModule], 0, 
function(S)
 # returns the decomposition numbers of the given sub Weyl module
